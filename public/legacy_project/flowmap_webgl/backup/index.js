...
getJSON('boundary/iowa.json', function (data) {
    ...
    for (let i = 0; i < data.features.length; i++) {
        ...
        for (let j = 0; j < line.length; j++) {
			//console.log((line[j][0] + 180) * canvas.width / 360+" "+(-line[j][1] + 90) * canvas.height / 180);
			//console.log((1+Math.round((line[j][0]+97.154167)/dx))+" "+-(1-Math.round((line[j][1]-44.53785)/dy)));
			ctx[j ? 'lineTo' : 'moveTo'](
                //(line[j][0] + 180) * canvas.width / 360, 791
                //(-line[j][1] + 90) * canvas.height / 180);	 358			
				(1+Math.round((line[j][0]+97.154167)/dx)*canvas.width/1741) ,
                -(1-Math.round((line[j][1]-44.53785)/dy)*canvas.height/1057));				
        }
    }
    ctx.stroke();
});
