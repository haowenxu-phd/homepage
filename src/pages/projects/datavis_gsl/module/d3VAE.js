...
import {EventTagGetEventArray, EventTagHierarchyGrouping} from './d3EventTagGrouping.js'

var showPreviousLatent = "";
var EventTagHierarchyGroupingRes = {};
...
export function VaeScatterPlot(data, type_event, childChangeParent) {
  ...
  let container = d3.select("#dataviz_axisZoom").node();
    ...
    var margin = {top: 10, right: 30, bottom: 20, left: 30},        
        ...
        let EventTagGetEventArrayRes = EventTagGetEventArray(data,3);
        //console.log("EventTagGetEventArrayRes",JSON.stringify(EventTagGetEventArrayRes));
        EventTagHierarchyGroupingRes = EventTagHierarchyGrouping(EventTagGetEventArrayRes,20,3);
        //console.log("EventTagHierarchyGroupingRes",EventTagHierarchyGroupingRes);
    ...
    const svg = d3.select("#dataviz_axisZoom").select("svg");
        ...
        if (!svg.empty()) {
          ...
          filterByEvent(".pie-chart",colorRamp,colorRamp2,"change_color_true",EventTagHierarchyGroupingRes);
          filterByEvent(".d3_circle",colorRamp,colorRamp2,"change_color_true",EventTagHierarchyGroupingRes);
    ...
    function parseEventForClassKeyChangeStyle(eventText) {
      ...
      res.forEach(function(item) {
          ...
          d3.selectAll(`.${className.split("--")[0]+"--"+className.split("--")[1]}`)
              ...
              // Select all elements matching the modified class name and change their style   
          d3.selectAll(`.${className}_t`)
          .style("opacity", 1); // Set desired opacity level

          // Select all elements matching the modified class name and change their style   
          d3.selectAll(`.${className.split("--")[0]}_t`)
              .style("opacity", 1); // Set desired opacity level

          d3.selectAll(`.${className.split("--")[0]+"--"+className.split("--")[1]}_t`)
              .style("opacity", 1); // Set desired opacity level
  ...
  // Append groups instead of circles
    ...
    .on("mouseover", function(event, d) {
        ...
        tooltip.html( 
            ...
            <b>GMM Prob:</b><br>
            ${ Number(dt.cluster_prob).toFixed(2) || "N/A"}<br>
            <b>GMM Label:</b><br>
            ${dt.cluster_label || "N/A"}<br>
            <b>HDBSCAN Label:</b><br>
            ${dt.hdbscan_minsize3_cluster_label || "N/A"}
            `
        );
    ...
    .on("mouseout", function(event, d) {
        ...
        /*d3.selectAll(`.d3_tree_boxes`).style("opacity", 0.2);   
                    ...
                    d3.selectAll(`.d3_tree_boxes`).style("stroke", "none");  */
    });
    ...
    function drawPieChart(selection, colorScale, dtp) {
      ...
      allClassEventsArray.forEach(function(item, index) {
        ...
        data.push({
          ...
          cluster_label: dtp["cluster_label"],
          hdbscan_minsize3_cluster_label: dtp["hdbscan_minsize3_cluster_label"],
          eventTags: dtp["eventTags"] 
        });
      });
    
      const pie = d3.pie().value(d => d.value);
      const arc = d3.arc()
        .innerRadius(7)  // Inner radius for a ring; set to 0 for a pie chart
        .outerRadius(23 + Number( parseDuration(dtp.waveDuration)) * 12);
      ...
      const arcs = selection.selectAll("path")
        ...
        .style("stroke", function(d){
          if(color_type=="event_group"){
            ...
            let clusterID = EventTagHierarchyGroupingRes[dtp["eventTags"]] || '-1';
            //console.log(d["eventTags"],EventTagHierarchyGroupingRes[d["eventTags"]])
            return colorRamp(clusterID) || "black";
            //return colorRamp(showColorClass(d)) || "black"}
          } 
          if(color_type=="GMM_cluster"){
            return colorRamp2(dtp.cluster_label) || "black"
          }
          if(color_type=="hDBSCAN_cluster"){
            return colorRamp2(dtp.hdbscan_minsize3_cluster_label) || "black"
          }
        }) 
      ...
      selection.selectAll("text")
        ...
        .text(function(d){
          ...
          return className.at(-2)[0]+className.at(-1)[0];
        }
...
function drawSVGCirclePlots(scatter,data){
  ...
  .attr("class", "d3_circle")
    ...
    .style("fill", function(d){      
      ...
      if(color_type=="hDBSCAN_cluster"){
        return colorRamp2(d.hdbscan_minsize3_cluster_label) || "black"
      }

      if(color_type=="event_group"){

        let clusterID = EventTagHierarchyGroupingRes[d["eventTags"]] || '-1';
        //console.log(d["eventTags"],EventTagHierarchyGroupingRes[d["eventTags"]])
        return colorRamp(clusterID) || "black";
        //return colorRamp(showColorClass(d)) || "black"}
      } 
