...
export function filterByEvent(className,colorRamp,colorRamp2,change_color_bool,EventTagHierarchyGroupingRes){
    ...
    if(change_color_bool=="change_color_true"){
        d3.selectAll(".d3_circle")             
          ...
          d3.selectAll(".pie-chart").selectAll("path")                
          //.style("fill", function(d){return colorRamp(showColorClass(d)) || "gray"})
          .style("stroke", function(d){     
            //console.log(d); 
            if(color_type=="GMM_cluster"){
              return colorRamp2(d["data"].cluster_label) || "black" 
            }
            if(color_type=="hDBSCAN_cluster"){
              return colorRamp2(d["data"].hdbscan_minsize3_cluster_label) || "black" 
              
            }
            if(color_type=="event_group"){
              //return colorRamp(showColorClass(d["data"])) || "black"
              let clusterID = EventTagHierarchyGroupingRes[d["data"]["eventTags"]] || '-1';
              return colorRamp(clusterID) || "black";
  ...
  export function showColorClass(d){  
    ...
    res.forEach(function(item2,index){
      ...
      ///if( item2.includes(val)){
        item2=item2.split("::").slice(2,3).join("::") 
