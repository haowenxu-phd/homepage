...
function drawGridDots(gridData,eventTagsCount, eventCorrelationArray) {
      ...
      row.forEach((key, colIndex) => {
          if (key !== null) {
              ...
              gridGroup.append("rect")
                ...
                d3.selectAll(".d3_tree_boxes")
                  .on("mouseover", function(event, d) {
                    ...
                    let matchingText = matching;
                    changeD3VAEScatter(".pie-chart", matchingText);
                    changeD3VAEScatter(".d3_circle", matchingText);  
                    console.log(matching)
                     
                    //console.log(event, d);
                    //d3.selectAll(`.${matching} rect`).style("stroke", "red");   

                    let matching2 = d3.select(this).attr("data2");
                    //console.log(matching2);
                    let correlatArray = []
                    eventCorrelationArray.forEach(function(item,index){
                        if(item[0].includes(matching2)){
                            //console.log(item[0]);
                            let eventArray = item[0].split("/r/n");
                            eventArray.forEach(function(it2,idx2){
                                if(correlatArray.indexOf(it2)==-1){
                                    correlatArray.push(it2);
                                }
                            })                            
                        }                        
                    });
                    //console.log(correlatArray);
                    correlatArray.forEach(function(itm3,idx3){
                        const className2 = itm3.split("::").join("--").split(" ").join("_")
                        .split("&").join("_nd_").split("/").join("_or_");
                        //console.log(className2)
                        d3.selectAll(`.${className2}`).style("opacity", 0.8);  
                        d3.selectAll(`.${className2}_t`).style("opacity", 0.8);  
                    })
                    d3.selectAll(`.${matchingText}`).style("opacity", 1);  
                    d3.selectAll(`.${matchingText}_t`).style("opacity", 1);  
                    d3.selectAll(`.${matchingText}`).style("stroke", "black");  
              ...
              gridGroup.append("text")
                  ...
                  .style("opacity", 0.3)
                  .attr("class", `${className}_t d3_tree_text`)
              ...
              gridGroup.append("text")
                  ...
                  .style("font-size", "9px")
                  .style("opacity", 0.5)
                  .attr("class", `${className}_t d3_tree_text_count`)
                  .style("pointer-events","none")
                  .style("font-weight", "bold")
...
export function createHierachyPlot(data){
    ...
    //console.log(grid);
    drawGridDots(grid,eventTagsCount,EventTagGetEventArrayRes);
