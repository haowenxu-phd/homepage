// basicShapes.js
function addVoxelPlane(renderer, renderWindow, dims, z = 0) {
  const [nx, ny, nz] = dims;
  console.log("dim", dims);
  const plane = vtk.Filters.Sources.vtkPlaneSource.newInstance({
    XResolution: 1,
    YResolution: 1,
    origin:  [500, 500, z],     // lower-left   //not working
...
function addVoxelCube(renderer, renderWindow, dims = [500, 500, 200]) {
  const [nx, ny, nz] = dims;
