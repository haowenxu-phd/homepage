            ...
            <div className="absolute top-3 left-3 z-10 bg-black/70 text-white text-xs px-3 py-2 rounded max-h-[90%] overflow-y-auto">
              ...
              {selectedVoxel && (
                <div className="mt-2 text-green-300">
                  Selected voxel:{" "}
                  {selectedVoxel.i}, {selectedVoxel.j}, {selectedVoxel.k}
                </div>
              )}

              {windScenario && (
                <div className="mt-2 text-blue-300">
                  Wind: {windScenario.wind?.direction ?? "unknown"}{" "}
                  {windScenario.wind?.speed_mps ?? ""} m/s
                </div>
              )}
              ...
              <div className="mt-2">
                Views:
                <button
                  className="ml-1 px-2 bg-slate-700 rounded"
                  onClick={() => handleSnapView("x", +1)}
                >
                  +X
                </button>
                <button
              ...
              {fireTimesteps.length > 0 ? (
                <div className="mt-2">
                  ...
                  <input
                    ...
                    type="range"
                    min={0}
                    max={fireTimesteps.length - 1}
                    value={fireTimestepIndex}
                    step="1"
                    onChange={(e) => {
                      const idx = Number(e.target.value);
                      setFireTimestepIndex(idx);

                      const t = fireTimesteps[idx];
                      applyFireFrame(t, framesRef.current[t]);
                    }}
                  />

                  <span className="ml-2">
                    t = {fireTimesteps[fireTimestepIndex]}
                  </span>

                  <button
                    className="ml-2 px-2 bg-slate-700 hover:bg-slate-600 rounded"
                    onClick={() => setIsFirePlaying((v) => !v)}
                  >
                    {isFirePlaying ? "⏸" : "▶"}
                  </button>
                </div>
